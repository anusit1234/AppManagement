...
$sql = "SELECT T1.total1 AS cash,
            ...
            FROM (SELECT sum(amount) AS total1 FROM daily_expenses t1 
                    ...
                    WHERE t1.user_id = '$id' and t1.payment_channel_id ='1' and  MONTH(t1.created)='$month' order by t1.created DESC) AS T1,
                 (SELECT sum(amount) AS total2 FROM daily_expenses t1 
                    inner join payment_channel_categories t2 on t1.payment_channel_id = t2.id
                    WHERE t1.user_id = '$id' and t1.payment_channel_id ='2' and  MONTH(t1.created)='$month' order by t1.created DESC) AS T2,
                 (SELECT sum(amount) AS total3 FROM daily_expenses t1 
                    inner join payment_channel_categories t2 on t1.payment_channel_id = t2.id
                    WHERE t1.user_id = '$id' and t1.payment_channel_id ='3' and  MONTH(t1.created)='$month' order by t1.created DESC) AS T3,
                 (SELECT sum(amount) AS total4 FROM daily_expenses t1 
                    inner join payment_channel_categories t2 on t1.payment_channel_id = t2.id
                    WHERE t1.user_id = '$id' and t1.payment_channel_id ='4' and  MONTH(t1.created)='$month' order by t1.created DESC) AS T4,
                 (SELECT sum(amount) AS total5 FROM daily_expenses t1 
                    inner join payment_channel_categories t2 on t1.payment_channel_id = t2.id
                    WHERE t1.user_id = '$id' and t1.payment_channel_id ='5' and  MONTH(t1.created)='$month' order by t1.created DESC) AS T5
            ";
